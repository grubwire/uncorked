...
extension Bottle {
    ...
    private func registryDisplayName(for exeURL: URL) -> String? {
    ...
    /// is missing.
    private func parseUninstallEntries() -> [UninstallEntry] {
    ...
    private static func parseRegValueLine(_ line: String) -> (name: String, value: String)? {
        // Pattern: "name"="value"
        ...
        while idx < line.endIndex {
            let c = line[idx]
            if c == "\\", line.index(after: idx) < line.endIndex {
    ...
    private static func winePathToHostPath(_ winePath: String, driveCHostPath: String) -> String {
        var s = winePath
        if s.count >= 2 {
            let second = s.index(s.startIndex, offsetBy: 1)
            if s[second] == ":" {
                s = String(s.suffix(from: s.index(after: second)))
            }
        }
