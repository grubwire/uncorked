...
└── UncorkedKit (Swift package, local dependency)
    ├── Engine/         - UncorkedEngine, RosettaCheck, EngineManifest, bin path management
...
Engine hosted at: data.grubwire.io/engine/
  manifest.json       - {version, url, sha256}, Ed25519-signed
  manifest.json.sig   - 64-byte raw Ed25519 signature
  staging/manifest.*  - staging copies (same content, for DEBUG builds)
  <tag>/engine.tar.xz - Gcenx archive, re-signed by Grubwire CI

Engine installed at: ~/Library/Application Support/Uncorked/Libraries/Wine/
  bin/    - wine64, wineserver, wineboot, uncorked64 (wrapper), etc.
...
2. Human reviews the Gcenx release, then runs `engine-bundle.yml` via Actions tab.
3. `engine-bundle.yml`: downloads from Gcenx, generates wrappers, signs all Mach-O files
   (Phase 1: ad-hoc), repacks, computes SHA-256, generates and signs `manifest.json`,
   uploads to R2 (Cloudflare), commits `engine-version.txt`, opens a tracking issue.
4. Human tests (fresh install, bottle creation, known .exe), ships a Uncorked release.
5. App fetches manifest on first launch, downloads engine, verifies SHA-256, extracts to
   Application Support, clears quarantine.

## Engine Manifest Signing

The manifest is signed with Ed25519 (CryptoKit `Curve25519.Signing`).
...
- Private key (hex seed): stored in GitHub secret `ENGINE_MANIFEST_SIGNING_KEY`
  ...
  - To sign a manifest: `bash scripts/sign-manifest.sh manifest.json`
...
Asset priority: `wine-staging-*` > `wine-devel-*` > any `.tar.xz`
...
### Phase 2 (future — requires Apple Developer account, not yet set up)
...
Uncorked version is fully decoupled from engine version. Engine version never appears in
Uncorked's version string.

- Engine update: patch bump (1.0.2 → 1.0.3)
- Currently published engine tag recorded in `engine-version.txt` (repo root)
- In-app engine version stored in `UncorkedEngineVersion.plist` (under Libraries, diagnostics only)
...
| `scripts/sign-manifest.sh` | Signs manifest.json with Ed25519 private key |
| `scripts/generate-wrappers.sh` | Generates uncorked64, uncorkedserver, uncorkedboot |
| `engine-version.txt` | Currently published engine tag (updated by CI) |
| `.github/workflows/engine-bundle.yml` | Full publish pipeline (Gcenx → R2) |
...
## Pending: Checkpoint D (R2 / data.grubwire.io setup)

**Requires Nick's Cloudflare account.** Not yet configured.

Steps needed:
1. Create an R2 bucket (e.g. `uncorked-engine`)
2. Create a Cloudflare Worker or public bucket URL at `data.grubwire.io`
...
5. Run `engine-bundle.yml` for the first time to publish the engine
6. Verify the app can download the engine in a fresh install

Until Checkpoint D is done, new installs will fail at the engine download step (EngineSetupView
shows the error and a Retry button).

## Migration (existing users)

On first launch after upgrade, `removeLegacyEngineIfNeeded()` runs once and deletes the stale
engine at `Libraries/Wine` only if `UncorkedEngineVersion.plist` is absent (indicating the
pre-managed engine, not a freshly downloaded managed one). Bottles and wineprefixes are preserved.

## Internal Naming Rules

- All Swift code calls `uncorked64` by absolute path, never `wine64` directly.
...
- Winetricks is a proper noun kept throughout (not renamed).
- `wineVersion` remains the persisted `BottleSettings` key (backward compat with existing bottles).

## Sparkle Phase 1 to Phase 2 Transition
